...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		//Main.daScene.screen.cameras = [camHUD];
		//Main.daScene.screen.antialiasing = true;
		//add(Main.daScene.screen);

		/*Main.daScene.screen.cameras = [camHUD];
		Main.daScene.screen.antialiasing = true;
		add(Main.daScene.screen);
		
		var lTex:TextureMaterial = new TextureMaterial(Cast.bitmapTexture("assets/models/leftn.png"));
		lTex.alphaBlending = true;

		testNote = new Note3D(0,4,lTex);*/
		//testNote.sPlane.x += FlxG.width / 2;
		//testNote.sPlane.y += FlxG.height / 2;
	...
	public static function copyInfo(){
		PlayState3D.curStage = curStage;
		PlayState3D.isPixelStage = isPixelStage;
		PlayState3D.SONG = SONG;
		PlayState3D.isStoryMode = isStoryMode;
		PlayState3D.storyWeek = storyWeek;
		PlayState3D.storyPlaylist = storyPlaylist;
		PlayState3D.storyDifficulty = storyDifficulty;
		PlayState3D.practiceMode = practiceMode;
		PlayState3D.usedPractice = usedPractice;
		PlayState3D.changedDifficulty = changedDifficulty;
		PlayState3D.cpuControlled = cpuControlled;
		PlayState3D.campaignScore = campaignScore;
		PlayState3D.campaignMisses = campaignMisses;
		PlayState3D.seenCutscene = seenCutscene;
		PlayState3D.deathCounter = deathCounter;
	}
